...
tindex.prototype._set = function (k, v, o) {
	if (this._sub) {
		var s = k === null ? this._nuls[v] : this._bp.get(k);
		if (!s) {
			s = new tindex(this._sub, this._c, this.options, this.name + '_' + k);
			if (k === null) this._nuls[v] = s;
...
tindex.prototype.all = function (order) {
	var a = this._bp.all();
	var n = _.keys(this._nuls);
	var r = this.order > 0 ? _.union(a, n) : _.union(n, a);
	if (order && order.length > 0) {
		if (order[0] != this.order) r = r.reverse();
		order = order.slice(1);
	}
	if (this._sub) return _.flatten(_.map(r, function (i) { return i.all(order); }));
