...
var ops = {
	...
	$in : function () {
		this.op = "$in";
		this._index = function (index) {
			var u = _.uniq(_.map(this._args.slice(1), function (v) { return v._get(); }));
			return _.flatten(_.map(u, function (v) { return index.match(v); }), true);
		},
		this._ex = function (f) {
			if (this.fields()[f]!=null)
				return 1;
	...
	$nin : function () {
		...
		this._index = function (index) {
			var u = _.uniq(_.map(this._args.slice(1), function (v) { return v._get(); }));
			var x = _.flatten(_.map(u, function (v) { return index.match(v); }), true);
