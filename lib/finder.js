	...
	this.native = function () {
		...
		for (var i=0;i<path.length;i++) {
			...
			if (i!=path.length-1)
				...
				s+= "("+ps+" && "+ps +".$oid) || (" + ps + " instanceof Date && " + ps + ".getTime()) || "+ ps +")";
		}		
		return s;
	}	
	this.native3 = function () {
		function steep(ps, path, i) {
			...
			if (path.length!=i) {
				...
				s+= "res.push(v"+i+".$oid || (v" + i + " instanceof Date && v" + i + ".getTime()) || v"+i+")\n";
			return s;
		};
			
		var path = k.split(".");
		if (path.length==1)
			return "[(obj." + k + " && obj." + k + ".$oid) || (obj." + k + " instanceof Date && obj." + k + ".getTime()) || obj."+k+"]";
		else {
			var s = "(function () {\n";
			var ps = "obj";
			s+="var res=[]\n";
...
var value = function (v) {
	this.op = "v";
	if (v instanceof tdb.ObjectID)
		this.v = v.$oid || v.toJSON();
	else if (_.isDate(v))
		this.v = v.getTime();
	else
		this.v = v;
...
function stree(query,ctx) {
	...
	if (_.isArray(query)) {
	...
	} else if (_.isObject(query) && !(query instanceof tdb.ObjectID || _.isDate(query) || _.isFunction(query))) {
		_.each(query, function (v,k) {
			...
			if (op!=null) {
				...
				if (_.isObject(v) && !(v instanceof tdb.ObjectID || _.isDate(v) || _.isFunction(query))) {
