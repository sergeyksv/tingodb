...
tdb.prototype._collection = function (cname, opts, create, cb) {
	...
	if (c) {
		cb((opts.strict && create) ? new Error("Collection " + cname + " already exists. Currently in safe mode.") : null, c);
		return c;
	} else if (!create && opts.strict) {
		return cb(new Error("Collection " + cname + " does not exist. Currently in safe mode."));
	}
	c = new tcoll();
