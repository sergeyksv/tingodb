...
tcoll.prototype.findOne = function (query, opts, cb) {
	if (_.isFunction(opts) && cb == null) {
		cb = opts;
		opts = {};
	}	
	this.find(query,opts).limit(1).nextObject(cb);
}

tcoll.prototype.find = function (query, opts, cb) {
	if (_.isFunction(opts) && cb == null) {
		cb = opts;
		opts = {};
	}
	var c = new tcursor(this,query,opts);
	if (cb)
		cb(null, c)
	else
		return c;
