...
tcoll.prototype.createIndex = tcoll.prototype.ensureIndex = function (obj, options, cb) {
	...
	if (self._idx[key])
		return cb();
	var index = new tindex(key, self, options)
...
tcoll.prototype._ensureIds = function(obj) {
	...
	_.each(obj, function (v,k) {
		if (k.length >0 && k[0]=='$') {
			throw new Error("Document keys can't start from $");
		}
		if (_.isObject(v)) {
	...
	_.each(obj, function (v,k) {
		if (_.isObject(v)) {
			switch (v.$wrap) {
				...
				case "$oid":	obj[k] = new ObjectID(v.v); break;
...
tcoll.prototype._put = function (item, remove, cb) {
	...
	self._wq.add(function (cb) {	
		...
		var key = {_id:(item._id.$oid || item._id),_uid:self._id};	
		if (remove)	key._a="del";
		item = self._wrapTypes(item);			
		var sobj = remove?"":JSON.stringify(item);
...
tcoll.prototype.findAndModify = function (query, sort, doc, opts, cb) {
	...
	c.sort(sort);
	if (c._err)
		return process.nextTick(function () { cb(c._err) });
