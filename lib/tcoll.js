...
tcoll.prototype._compact = function (cb) {
	...
	fs.open(filename, 'w+', safe.sure(cb, function (fd) {
		...
		async.forEachSeries(_.keys(self._store), function (k, cb) {
			var pos = self._store[k];
			get(pos, safe.sure(cb, function (data) {
				fs.write(fd, data, 0, data.length, wpos, safe.sure(cb, function () {
					store[k] = wpos;
					wpos += data.length;
					cb();
				}));
...
tcoll.prototype.__find = function (query, fields, skip, limit, sort_, order, arFields, cb) {
	...
	if (self._tdb._gopts.searchInArray != false)
		arrayMatch = true;
	else {
...
tcoll.prototype.group = function (keys, condition, initial, reduce, finalize, command, options, callback) {
	...
	with (options.scope || {}) {
		try {
			if (_.isFunction(keys) || keys instanceof Code) keys = eval('(' + keys + ')');
			reduce = eval('(' + reduce + ')');
