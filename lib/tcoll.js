...
tcoll.prototype.__find = function (query, fields, skip, limit, sort, hint, arFields, cb) {
	...
	if (!_.isEmpty(pi)) {
		...
		function i2i(v, i) {
			var r = v._index(i);
			// for compound indexes convert subindexes to plain ids
			if (i._sub) r = _.flatten(_.map(r, function (si) {
				var v = io[si.key];
				return v ? i2i(v, si) : si.all();
			}));
			return r;
		}
