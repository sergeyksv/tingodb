...
tcoll.prototype._bestSearchIndex = function (pi) {
	var fields = {};
	var best = {};
	_(pi).forEach(function (arr, k) {
		// skip this key if we already have (compound) index for it
		if (fields[k]) return;
		// sort indexes by weight
		arr = arr.sort(function (l, r) { return r.w - l.w; });
		// get best index
		var i = arr[0].i;
		best[k] = i;
		// note that we have index for all subfields
		_(i.fields()).forEach(function (f) {
			fields[f[0]] = true;
		});
	});
	return best;
...
tcoll.prototype.__find = function (query, fields, skip, limit, sort, hint, arFields, cb) {
	...
	if (!_.isEmpty(pi)) {
		...
		pi = this._bestSearchIndex(pi);
		// build query plan
		var qp = [];
		_(pi).forEach(function (v) {
			var st = { idx: v, io: [] };
			_(v.fields()).forEach(function (f) {
				var k = f[0];
				if (qt._ex(k) == 1) st.io.push(qt.split(k));
				else {
					st.e = true; // should expand subindexes to plain ids
					return false;
				}
			});
			qp.push(st);
		...
		// execute query plan
		var p = [];
		_(qp).forEach(function (st) {
			// this action applies to all indexes
			var r = st.io[0]._index(st.idx);
			// process subfields of compound index
			_(st.io.slice(1)).forEach(function (io) {
				r = _.flatten(_.map(r, function (si) { return io._index(si); }));
			});
